...
void init_player_tab(Player *player_tab, int n_player, int n_peng_per_player) {
  ...
  assert(n_peng_per_player >= N_MIN_PENGUINS_PER_PLAYER &&
         n_peng_per_player <= N_MAX_PENGUINS_PER_PLAYER);
  ...
  for (int i = 0; i < n_player; i++) {
    ...
    do {
      ...
      player_tab[i].penguins = malloc(sizeof(Penguin) * n_peng_per_player);
      verify(player_tab[i].penguins != NULL, "erreur d'allocation memoire");
      for (int j = 0; j < n_peng_per_player; j++) {
 ...
 * @param n_peng_per_player	number of penguins to verify for the player
 * @return true if the player is valid
 */
bool player_is_valid(Player *player, unsigned int n_peng_per_player) {
  if (player == NULL) {
    return false;
  }
  // general datas
  if (player->score < 0 || n_peng_per_player < 2 || n_peng_per_player > 5) {
    return false;
  } else if (player->penguins == NULL) {
    return false;
  }
  // penguin datas
  for (int i = 0; i < n_peng_per_player; i++) {
